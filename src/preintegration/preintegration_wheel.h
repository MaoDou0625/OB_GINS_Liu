...
#ifndef PREINTEGRATION_WHEEL_H
#define PREINTEGRATION_WHEEL_H

#include "preintegration_base.h"

// Wheel IMU preintegration with lever-arm dynamics compensation.
// Maps wheel-IMU measurements to vehicle body origin using:
// f_w^b = f_O^b + dot(omega)^b x l + omega^b x (omega^b x l)
class PreintegrationWheel : public PreintegrationBase {

public:
    PreintegrationWheel(std::shared_ptr<IntegrationParameters> parameters,
                        const IMU &imu0,
...
private:
    ...
    Matrix3d Rbw_ = Matrix3d::Identity();
    Vector3d lever_ = Vector3d::Zero();
};

#endif // PREINTEGRATION_WHEEL_H
