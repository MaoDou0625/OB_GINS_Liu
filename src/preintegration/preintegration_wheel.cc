...
#include "preintegration_wheel.h"

PreintegrationWheel::PreintegrationWheel(std::shared_ptr<IntegrationParameters> parameters,
                                         const IMU &imu0,
                                         IntegrationState state,
                                         const Vector3d &rbw_angle,
                                         const Vector3d &lever)
    ...
    Rbw_  = Rotation::euler2matrix(rbw_angle);
    lever_ = lever;
}

Eigen::MatrixXd PreintegrationWheel::evaluate(const IntegrationState &state0, const IntegrationState &state1,
                                              double *residuals) {
    sqrt_information_ =
...
Eigen::MatrixXd PreintegrationWheel::residualJacobianPose0(const IntegrationState &state0,
...
Eigen::MatrixXd PreintegrationWheel::residualJacobianPose1(const IntegrationState &state0,
                                                           const IntegrationState &state1, double *jacobian) {
    Eigen::Map<Eigen::Matrix<double, NUM_STATE, NUM_POSE, Eigen::RowMajor>> jaco(jacobian);
    jaco.setZero();
...
Eigen::MatrixXd PreintegrationWheel::residualJacobianMix0(const IntegrationState &state0,
                                                          const IntegrationState &state1, double *jacobian) {
...
Eigen::MatrixXd PreintegrationWheel::residualJacobianMix1(const IntegrationState &state0,
                                                          const IntegrationState &state1, double *jacobian) {
    Eigen::Map<Eigen::Matrix<double, NUM_STATE, NUM_MIX, Eigen::RowMajor>> jaco(jacobian);
    jaco.setZero();
...
int PreintegrationWheel::numResiduals() { return NUM_STATE; }

vector<int> PreintegrationWheel::numBlocksParameters() { return vector<int>{NUM_POSE, NUM_MIX, NUM_POSE, NUM_MIX}; }

IntegrationStateData PreintegrationWheel::stateToData(const IntegrationState &state) {
    IntegrationStateData data;
    PreintegrationBase::stateToData(state, data);
    return data;
}

IntegrationState PreintegrationWheel::stateFromData(const IntegrationStateData &data) {
    IntegrationState state;
    PreintegrationBase::stateFromData(data, state);
    return state;
}

void PreintegrationWheel::constructState(const double *const *parameters, IntegrationState &state0,
...
void PreintegrationWheel::integrationProcess(unsigned long index) {
    IMU imu_pre = compensationBias(imu_buffer_[index - 1]);
    IMU imu_cur = compensationBias(imu_buffer_[index]);

    // Transform wheel IMU increments to body frame
    IMU imu_pre_b = imu_pre;
    IMU imu_cur_b = imu_cur;
    imu_pre_b.dtheta = Rbw_ * imu_pre_b.dtheta;
    imu_pre_b.dvel   = Rbw_ * imu_pre_b.dvel;
    imu_cur_b.dtheta = Rbw_ * imu_cur_b.dtheta;
    imu_cur_b.dvel   = Rbw_ * imu_cur_b.dvel;

    // Lever dynamics compensation (approximate, use current sample)
    double dt  = std::max(imu_cur_b.dt, 1e-6);
    Vector3d w0 = imu_pre_b.dtheta / std::max(imu_pre_b.dt, 1e-6);
    Vector3d w1 = imu_cur_b.dtheta / dt;
    Vector3d alpha = (w1 - w0) / dt;
    Vector3d acc_corr = alpha.cross(lever_) + w1.cross(w1.cross(lever_));
    imu_cur_b.dvel -= acc_corr * dt; // map f_w -> f_O

    // Integrate and update
    integration(imu_pre_b, imu_cur_b);
    updateJacobianAndCovariance(imu_pre_b, imu_cur_b);
}

void PreintegrationWheel::resetState(const IntegrationState &state) { resetState(state, NUM_STATE); }

void PreintegrationWheel::updateJacobianAndCovariance(const IMU &imu_pre, const IMU &imu_cur) {
    // Use same linearization as normal preintegration (approximation)
    ...
    Eigen::MatrixXd Qk = 0.5 * dt * (phi * gt * noise_ * gt.transpose() + gt * noise_ * gt.transpose() * phi.transpose());
    covariance_ = phi * covariance_ * phi.transpose() + Qk;
}

void PreintegrationWheel::resetState(const IntegrationState &state, int num) {
    delta_time_ = 0;
...
void PreintegrationWheel::setNoiseMatrix() {
    ...
    noise_.block<3, 3>(6, 6) *= 2 * parameters_->gyr_bias_std * parameters_->gyr_bias_std / parameters_->corr_time; // nbg
    noise_.block<3, 3>(9, 9) *= 2 * parameters_->acc_bias_std * parameters_->acc_bias_std / parameters_->corr_time;  // nba
}

int PreintegrationWheel::imuErrorNumResiduals() { return 6; }

vector<int> PreintegrationWheel::imuErrorNumBlocksParameters() { return std::vector<int>{NUM_MIX}; }

void PreintegrationWheel::imuErrorEvaluate(const double *const *parameters, double *residuals) {
...
void PreintegrationWheel::imuErrorJacobian(double *jacobian) {
