...
namespace {
const Matrix3d &WheelRbdToFrdMatrix() {
    static const Matrix3d rot = [] {
        Matrix3d m;
        m << 0, -1, 0,
             1,  0, 0,
             0,  0, 1;
...
WheelPreintegrationBase::WheelPreintegrationBase(std::shared_ptr<IntegrationParameters> parameters, const IMU &imu0,
                                       IntegrationState state)
    : parameters_(std::move(parameters))
    , current_state_(std::move(state)) {

    // Wheel IMU outputs are in RBD; convert to the FRD body frame.
    IMU imu_body = convertRbdToFrd(imu0);

    start_time_ = imu_body.time;
    end_time_   = imu_body.time;

    imu_buffer_.clear();
    imu_buffer_.push_back(imu_body);

    gravity_ = Vector3d(0, 0, parameters_->gravity);
}

void WheelPreintegrationBase::integration(const IMU &imu_pre, const IMU &imu_cur) {
    // 区间时间累积
...
void WheelPreintegrationBase::addNewImu(const IMU &imu) {
    imu_buffer_.push_back(convertRbdToFrd(imu));
    integrationProcess(imu_buffer_.size() - 1);
}

void WheelPreintegrationBase::reintegration(IntegrationState &state) {
    current_state_ = std::move(state);
...
IMU WheelPreintegrationBase::compensationBias(const IMU &imu) const {
    IMU imu_calib = imu;
...
IMU WheelPreintegrationBase::compensationScale(const IMU &imu) const {
    IMU imu_calib = imu;
...
void WheelPreintegrationBase::stateToData(const IntegrationState &state, IntegrationStateData &data) {
...
void WheelPreintegrationBase::stateFromData(const IntegrationStateData &data, IntegrationState &state) {
...
IMU WheelPreintegrationBase::convertRbdToFrd(const IMU &imu) {
    IMU converted = imu;
    const Matrix3d &rot = WheelRbdToFrdMatrix();
    converted.dtheta = rot * imu.dtheta;
    converted.dvel   = rot * imu.dvel;
    return converted;
}
